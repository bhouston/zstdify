...
function main(): void {
  ...
  console.log(
    `[decode-profile-fzstd-node] starting turns=${turns} payloads=${prepared.length} mode=${
  ...
  for (let i = 0; i < turns; i++) {
    ...
    const a = decompressFzstd(p.nodeCompressed);
    checksum = (checksum + (a[0] ?? 0)) | 0;
    p.turns++;
    ...
    if (completedTurns === turns || completedTurns % progressEveryTurns === 0) {
      ...
      console.log(
        `[decode-profile-fzstd-node] progress ${completedTurns}/${turns} (${pct}%) elapsed=${wallElapsedMs}ms eta=${Math.round(
          etaMs,
        )}ms`,
      );
    }
  ...
  const outPath = path.join(BENCH_DIR, 'decode-profile.fzstd-node.latest.json');
