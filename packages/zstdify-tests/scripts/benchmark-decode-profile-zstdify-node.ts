...
function main(): void {
  ...
  console.log(
    `[decode-profile-zstdify-node] starting turns=${turns} payloads=${prepared.length} mode=${
  ...
  for (let i = 0; i < turns; i++) {
    ...
    if (completedTurns === turns || completedTurns % progressEveryTurns === 0) {
      ...
      console.log(
        `[decode-profile-zstdify-node] progress ${completedTurns}/${turns} (${pct}%) elapsed=${wallElapsedMs}ms eta=${Math.round(
          etaMs,
        )}ms`,
      );
    }
  ...
  const outPath = path.join(BENCH_DIR, 'decode-profile.zstdify-node.latest.json');
